...
//Set this MPI task's halo values in the x-direction via neighborhood collectives
void set_exchange_halo_values_x( double *state ) {
  int k, ll, ind_r, ind_u, ind_t, i, s, ierr;
  double z;
  MPI_Request req_r[2], req_s[2];
      
  int counts[2]= {NUM_VARS*nz*hs, NUM_VARS*nz*hs};
  int displs[2] ={0,NUM_VARS*nz*hs};
  ...
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
        sendbuf[ll*nz*hs + k*hs + s]                      = state[ll*(nz+2*hs)*(nx+2*hs) + (k+hs)*(nx+2*hs) + hs+s];
        sendbuf[(NUM_VARS*nz*hs) + (ll*nz*hs + k*hs + s)] = state[ll*(nz+2*hs)*(nx+2*hs) + (k+hs)*(nx+2*hs) + nx+s];
      }
    }
  }
  ...
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      for (s=0; s<hs; s++) {
        state[ll*(nz+2*hs)*(nx+2*hs) + (k+hs)*(nx+2*hs) + s      ] = recvbuf[ll*nz*hs + k*hs + s];
        state[ll*(nz+2*hs)*(nx+2*hs) + (k+hs)*(nx+2*hs) + nx+hs+s] = recvbuf[(NUM_VARS*nz*hs) + ll*nz*hs + k*hs + s];
      }
    }
  }
...
//r,u,w,t are output density, u-wind, w-wind, and potential temperature at thaset location
